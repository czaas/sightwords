...
export const ChooseListType = (state, actions, data, emit) => {
  ...
  return (
    <ViewContainer state={state} actions={actions}>
      ...
      <p><a onclick={() => actions.router.go(`/list/all/${ state.currentWord.id }`)}>All words List <span>{mainListCount} / {(state.currentUser.list) ? state.currentUser.list.length : undefined}</span></a></p>
      <p>or</p>
